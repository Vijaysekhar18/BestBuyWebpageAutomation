...
public class ClarkUFunctinalities {
    WebDriver driver;
    ...
    void setup() {
        ...
        // Code id created as part of Project Phase 3
    ...
    void testLogoPresence () {
        try {
            // Code to locate the logo element
            WebElement logo = driver.findElement(By.xpath("//img[@alt='Clark University']"));
            // Verify that the logo is displayed
            assert logo.isDisplayed();
            System.out.println("Logo is displayed on the website.");
            // Get and print the text within the logo element
            String logoText = logo.getAttribute("alt");
            System.out.println("Text within the logo: " + logoText);
            System.out.println("Test case testLogo passed.");
        } catch (AssertionError e) {
            System.out.println("Logo is not displayed on the website.");
            System.out.println("Test case testLogo failed: " + e.getMessage());
            throw e; // Re-throw the exception to mark the test as failed.
        }
    }
    @Test
    void testMainSearchBoxFunctionality () {
        try {
            // Code to locate the searchbox element
            WebElement searchBox = driver.findElement(By.xpath("//button[contains(@class, 'search-icon-button')]"));
            ...
            // Sending the data to search
            searchInput.sendKeys("Software Engineering");
            // Press Enter key
            searchInput.sendKeys(Keys.RETURN);
            System.out.println("Test case testMainSearchBox passed.");
        } catch (AssertionError e) {
            System.out.println("Test case testMainSearchBox failed: " + e.getMessage());
            throw e; // Re-throw the exception to mark the test as failed.
        }
    }
    @Test
    void testCareerOutcomeSection() {
        try {
            // Code to locate the careerOutcome element
            WebElement careerOutcome = driver.findElement(By.xpath("//h4[@class='grid-info-box__title content' and text()='Career Outcomes']"));
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", careerOutcome);
            // Verify that the careerOutcome section is displayed
            assert careerOutcome.isDisplayed();
            System.out.println("careerOutcome is displayed on the website and able to click.");
            System.out.println("Test case testCareerOutcomeSection passed.");
        } catch (AssertionError e) {
            System.out.println("careerOutcome Section is not displayed on the website.");
            System.out.println("Test case testCareerOutcomeSection failed: " + e.getMessage());
            throw e; // Re-throw the exception to mark the test as failed.
        }
    }
    @Test
    void testCourseCatalogLink() {
        try {
            // Code to locate the careerOutcome element
            WebDriverWait wait = new WebDriverWait(driver, 10);
            WebElement courseCatalogLink = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[contains(@class, 'arrow-link') and contains(text(),'Course Catalog')]")));
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", courseCatalogLink);
            // Verify that the careerOutcome section is displayed
            assert courseCatalogLink.isDisplayed();
            courseCatalogLink.click();
            System.out.println("courseCatalogLink is displayed on the website and able to click on the link.");
            System.out.println("Test case testCourseCatalogLink passed.");
        } catch (AssertionError e) {
            System.out.println("courseCatalogLink  is not displayed on the website.");
            System.out.println("Test case testCourseCatalogLink failed: " + e.getMessage());
            throw e; // Re-throw the exception to mark the test as failed.
        }
    }
    @Test
    void testSocialMediaLinkLinkedin() {
        try {
            ...
            String expectedUrl = "https://www.linkedin.com/school/clark-university/";
            String actualUrl = driver.getCurrentUrl();
            assert actualUrl.equals(expectedUrl) : "Follow Us link did not navigate to the expected LinkedIn page";
            // Switch back to the main window
            driver.switchTo().window(mainWindowHandle);
            System.out.println("Test case testSocialMediaFollowLink passed.");
        } catch (AssertionError e) {
            System.out.println("Test case testSocialMediaFollowLink failed: " + e.getMessage());
            throw e; // Re-throw the exception to mark the test as failed
        } catch (InterruptedException e) {
    ...
    void testHelpfulEventsLink() {
        try {
            ...
            // Find and click the "Helpful Lik - events" link
            WebDriverWait wait = new WebDriverWait(driver, 30);
            WebElement eventsLink = driver.findElement(By.linkText("Events"));
            //WebElement eventsLink = driver.findElement(By.xpath("//a[contains(@href, 'https://www.clarku.edu/events/') and @aria-label='Events footer menu item' and contains(@class, 'focus-within')]\n"));
            eventsLink.click();
            Thread.sleep(3000);
            // Add an assertion to verify the URL after clicking the link
            String expectedUrl = "https://www.clarku.edu/events/";
            String actualUrl = driver.getCurrentUrl();
            assert actualUrl.equals(expectedUrl) : "Events Link did not navigate to the expected page";
            System.out.println("Test case testHelpfulEventsLink passed.");
        } catch (AssertionError e) {
            System.out.println("Test case testHelpfulEventsLink failed: " + e.getMessage());
            throw e; // Re-throw the exception to mark the test as failed
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }
    ...
    void testAdmissionLinkFunctionality  () {
        try {
            ...
            if (actualText.equals(expectedText)) {
                System.out.println("Text verification in Undergraduate Admissions page passed successfully!");
            } else {
